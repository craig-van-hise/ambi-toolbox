...
export async function generateProxy(filePath: string): Promise<Buffer> {
    return new Promise((resolve, reject) => {
        ...
        ffmpeg.stderr.on('data', (data) => {
            ...
            console.log(`[AmbiTrim Proxy]: ${data}`);
        });
        ...
        ffmpeg.on('close', (code) => {
            if (code === 0) {
                ...
                console.error(`[AmbiTrim] Proxy generation failed with code ${code}`);
                reject(new Error(`FFmpeg exited with code ${code}`));
            }
        });

        ffmpeg.on('error', (err) => {
            console.error(`[AmbiTrim] Failed to spawn FFmpeg: ${err.message}`);
            reject(err);
        });
    });
}
    ...
    return new Promise((resolve, reject) => {
        ...
        ffmpeg.on('close', (code) => {
            if (code === 0) {
                ...
                console.error(`[AmbiTrim] Trim failed with code ${code}`);
                reject(new Error(`FFmpeg exited with code ${code}`));
            }
        });
