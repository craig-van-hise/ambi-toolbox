...
export const handleAmbix2APAC = async (event: IpcMainInvokeEvent, options: { files: string[], bitrate: string }) => {
    ...
    if (bitrate.includes("64")) bitrateVal = 64000;
    if (bitrate.includes("96")) bitrateVal = 96000;
    if (bitrate.includes("128")) bitrateVal = 128000;
    if (bitrate.includes("192")) bitrateVal = 192000;
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        const outFile = path.join(fileDir, `${fileName}_apac.mp4`);
