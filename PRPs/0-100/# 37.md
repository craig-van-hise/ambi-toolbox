...
1. **No Rotation:** Output is flat.
2. **Data Loss:** File size shrank (Bit-depth reduction).
3. **Bad UX:** App creates a native modal/popup on success.

**Task:**

1. Replace `py/ambi_rotate.py` with a high-fidelity script.
2. Remove the modal from `index.tsx`.

**Step 1: The New Python Script (`py/ambi_rotate.py`)**
This script relies on `numpy` and `soundfile`.

* *Action:* Overwrite the existing file with this code.
...
**Step 2: Kill the Modal (`src/tools/AmbiRotate/index.tsx`)**
Find the `handleRender` function. It likely contains `window.electronAPI.showMessageBox` or `alert()`. **Delete that line.**
...
const handleRender = async () => {
    ...
    try {
        await window.electronAPI.processAmbiRotate(
            files.map(f => f.path), 
            { yaw, pitch, roll }
        );
        // Replace Modal with simple Console Log or a Toast if you have one
        console.log("Render Success"); 
        // Optional: Add a small "Done" text next to the button that fades out
    } catch (error) {
        console.error(error);
    } finally {
...
1. Overwrite the Python script. (This is the most critical step).
2. Update `index.tsx` to stop the annoyance.
3. **Run a Test Render.** The file size should now match the original (or be very close), and the audio should be rotated.